...
export function buildPrompt(intent, deliverable) {
  ...
  switch (skill) {
    ...
    case "vault-cleanup":
      // PRD C: allow-list scoping. Only genuinely ephemeral locations are
      // eligible; canon (Atlas), history (daily-notes), and runtime are named
      // as never-touch outright — defense in depth so the skill is safe to
      // schedule later. 30-day window so cleanup can't race the weekly
