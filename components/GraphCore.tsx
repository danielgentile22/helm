...
export default function GraphCore({
  ...
  useEffect(() => {
    ...
    let lastDeg = -1;
    let raf = 0;
    // Frame cap + pause-when-hidden. The HUD is meant to stay open, so an
    // uncapped loop renders the (expensive) bloom composer at the display's
    // full refresh — 120Hz on ProMotion — forever, which cooks the GPU.
    // 30fps is plenty for this slow ambient motion (dt-integrated sim below
    // keeps speed constant regardless of cap); hidden tabs render nothing.
    const reduceMotion =
    ...
    const tick = (nowMs = 0) => {
      ...
      if (nowMs - lastFrame < frameInterval) return;
      lastFrame = nowMs;
