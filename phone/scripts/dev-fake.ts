...
import { buildStack } from "../src/server/http/testkit";
...
const stack = await buildStack(script, home);
await cp(STATIC_DIR, join(home, "static"), { recursive: true });
(stack.agents as FakeAgentFactory).commandList = [
...
serve({ fetch: stack.fetch, hostname: "127.0.0.1", port: PORT }, (info) => {
