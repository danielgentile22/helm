...
HELM is push-to-talk by default; this is opt-in. No wake model ships with
HELM, so set WAKE_MODEL to an openWakeWord model name (e.g. one you trained
for "helm") and turn it on with WAKE_WORD=on. With no model set the listener
reports disabled via /health and the HUD stays push-to-talk. openWakeWord
rides the onnxruntime(-gpu) already in this venv — fully local, zero keys.
