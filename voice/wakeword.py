"""
Optional wake-word listener. openWakeWord on the default mic.

helm is push-to-talk by default; this is opt-in and off. No wake model
ships with helm, so set WAKE_MODEL to an openWakeWord model name (one you
trained for "helm", say) and turn it on with WAKE_WORD=on. With no model
set the listener reports disabled via /health. openwakeword and sounddevice
are imported lazily below so neither is a hard dependency while this is off.
...
Clients subscribe on ws://:3108/events: "wake" means barge-in (stop TTS,
show listening), "transcript" carries the recognized text.
"""
...
# No wake model ships with helm. Bring your own openWakeWord model name.
